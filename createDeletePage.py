...
import requests
from database import Database, addTransformer, removeTransformer
...
@st.dialog("Success")
def confirmed(name):
    st.write(f"{name} successfully deleted")
    close = st.button("Close")
    if close:
        st.rerun()
with col1:
    st.header("Create Transformer")
    with st.form("new_xfmr_form"):
        ...
        winding_material = st.text_input("winding_material")
