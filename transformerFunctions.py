...
class Transformer:
    def __init__(self,
        ...
        self.db = database
        self.name = name
        # self.transformerData = openpyxl.load_workbook(filename ="/home/eveuio/DataProcessing/TransformerData/"+self.name+".xlsx")
        ...
        # print(self.name +" rated time constant in hours: ", self.ratedTimeConstant)
        # print("")
    ...
    def avgWindingTemp(self,dayNumber):
        ...
        transformerData = pandas.read_sql_query(f'''SELECT * FROM {table_name}''',self.db.dbconnect)
        # dates = [cell.value for cell in transformerData['A'][4:-1]]
    ...
    #--------------------------------FRONT-END----------------------------------#
    #! Creates average reports(spreadsheet,database insert and graph) for current/voltage/etc with the choice of by day/hour. Full range of data availible
    def createAverageReport(self):
        table_name= self.name+"fullRange"
        transformerData = pandas.read_sql_query(f'''SELECT * FROM "{table_name}"''',self.db.dbconnect)
        ...
        transformerData['HS_AVG'] = numpy.sqrt((transformerData[hsTempA]+transformerData[hsTempB]+transformerData[hsTempC])/3)
        ...
        hourly_avg.to_sql(name= f'''{self.name}_hourlyTest''',con=self.db.dbconnect,if_exists = "replace",chunksize=5000,method ="multi", index=True, index_label = "DATETIME")
        daily_avg.to_sql(name= f'''{self.name}_dailyTest''',con=self.db.dbconnect,if_exists = "replace",chunksize=5000,method ="multi",index=True, index_label = "DATETIME")
